...
/*--------------------------------------------------------------*/
/* Helper function to fetch group data by level */
/*--------------------------------------------------------------*/
function find_group_data_by_level($level)
{
  global $db;
  $sql = "SELECT * FROM user_groups WHERE group_level = '{$db->escape($level)}' LIMIT 1";
  $result = $db->query($sql);
  return $db->fetch_assoc($result); // Returns array or null
}
...
function page_require_level($require_level){
  ...
  $login_level = find_group_data_by_level($current_user['user_level']);
